...
const mockTools: Record<string, (args: Record<string, string>) => string> = {
  searchClinicalTrials: ({ query, ageGroup }) =>
    JSON.stringify({
      totalFound: 3,
      trials: [
        {
          ...
          enrollment: 420,
          summary:
            "Evaluated renal outcomes in patients 65+ on combined metformin and ACE inhibitor therapy.",
        },
        {
          nctId: "NCT03891225",
          ...
          enrollment: 780,
          summary:
            "Post-marketing study on metformin safety in moderate-to-severe kidney disease.",
        },
        {
          nctId: "NCT05217890",
          ...
          enrollment: 156,
          terminationReason:
            "Higher than expected rate of lactic acidosis events in the eGFR <30 subgroup.",
        },
      ],
    }),
  ...
  getDrugLabel: ({ drugName }) => {
    const labels: Record<string, object> = {
      Metformin: {
        ...
        geriatricUse:
          "Elderly patients are more likely to have decreased renal function. Dose selection should be cautious. Monitor renal function more frequently.",
        renalDosing:
          "eGFR 30-45: not recommended to initiate. eGFR <30: contraindicated.",
      },
      Lisinopril: {
        contraindications: [
        ...
        geriatricUse:
          "Greater sensitivity of older individuals cannot be ruled out. Adjust dose based on renal function.",
      },
    };
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Search terms, e.g. drug names, conditions",
          },
          ageGroup: {
            type: "string",
            description: "e.g. 'elderly', 'adult', 'child'",
          },
        },
        required: ["query"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          reaction: {
            type: "string",
            description: "Optional: filter by specific reaction type",
          },
        },
        required: ["drugName"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          drugName: {
            type: "string",
            description: "Drug name to get label for",
          },
        },
        required: ["drugName"],
      },
  ...
  for (let step = 1; step <= MAX_STEPS; step++) {
    ...
    if (message.tool_calls && message.tool_calls.length > 0) {
      ...
      for (const tc of message.tool_calls) {
        ...
        const result =
          mockTools[tc.function.name]?.(args) ?? '{"error": "Unknown tool"}';
